if Config.Carjack_Framework == 'esx' then 
    ...
    Carjack = function()
        ...
        if closestPed ~= ped and closestPedDist < 2 then
            ...
            if vehicle ~= 0 then
                ...
                if Config.Carjack_AlwaysUnlock then
                    local plate = GetVehicleNumberPlateText(vehicle)
                    SetVehicleDoorsLocked(vehicle, 0)
                    Unlock(vehicle, plate)
                end
                if Config.Carjack_FreezeVehicle then
...
elseif Config.Carjack_Framework == 'qb' then
...
if Config.Carjack_Target == 'qtarget' then
        ...
        options = {
            {
                ...
                canInteract =     function()
                   return Config.Carjack_TakeVehWeapons[GetSelectedPedWeapon(PlayerPedId())] ~= nil
                end,
                action = function()
                    Carjack()
                end
...
elseif Config.Carjack_Target == 'ox_target' then
...
elseif Config.Carjack_Target == 'qb-target' then
    ...
    RegisterCommand('carjack', function()
        if Config.Carjack_TakeVehWeapons[GetSelectedPedWeapon(PlayerPedId())] ~= nil then
          Carjack()
        end
    end)   
end
